...
A side-panel tool for cold LinkedIn outreach. Sits next to LinkedIn on your screen: pick the prospect's persona, pick the next message in their sequence, copy it, paste into your LinkedIn message. Built to get you to the right message in under 5 seconds across 75+ daily touches.
...
```
index.html     The whole app — persona data is baked in. Just open this.
manifest.json  PWA manifest (optional, for installing as an app).
.gitignore
README.md
```

`index.html` is fully self-contained, so it works whether you open it locally or host it on GitHub Pages — nothing else needs to load.
...
3. The subject and body each have a one-click **Copy** button; **Copy full message** grabs the body in one tap. `[Name]` is highlighted orange — swap it after pasting.
4. Paste into your LinkedIn message and send.
...
**Add tab** — write a one-off custom message, preview it with `[Name]` highlighted, and copy. Not saved (edit `index.html` for permanent messages).
...
The persona data lives in a `<script>` block near the bottom of `index.html` (look for `const PERSONA_SOURCE`). Each persona lists messages 1–4 explicitly. Messages 5 and 6 (soft intro + soft ask) come from the shared `introMsg` / `askMsg` templates above it, filled with each persona's `audience` word — edit a template to change them for everyone, or use the Edit tab to change one persona.
...
- Edits made in the Edit tab live in your browser's local storage (key `inmail_edits_v1`). They follow the browser, not the file — clearing site data wipes them. Permanent copy belongs in the `PERSONA_SOURCE` block in `index.html`.
- Every button just copies to your clipboard — paste into LinkedIn yourself.
